...
\m5
   ...
   /
   / 4-TEAM TOURNAMENT RUN  (batch grid + subset seating + points scoring)
   /
   / All four enlisted teams are instantiated ONCE (the roster). Every game seats 4 of
   / them (a "4-team game"). We play every ordered seating (all 24 permutations of 4 teams
   ...
   / once. Which 4 of the 4 teams are seated -- and in what turn order -- is chosen per game by
   // *seat_map (a runtime lookup via the per-board $lineup index), so all 4 team circuits are
   / built once yet only 4 play any given game (num_seats=4 decouples seats from the roster).
   /
   / Scoring: only the SOLE winner of a game scores, 7 point(s) each (a 4-team game is worth
   / 7). Draws (timed out) score nothing. The scoreboard under the grid shows each team's
...
\TLV
   ...
   m5_var(num_lineups, 24)        // distinct ordered seatings (permutations of 4 of the 4 teams)
   m5_var(num_seats, 4)          // players seated per game (K); < roster => subset seating
   ...
   // 0..3, flattened as [lineup * 4 + seat]. A per-board $Lineup selects the seating at runtime.
   \SV_plus
      logic[1:0] *seat_map[0:95] = {
         2'd0, 2'd1, 2'd2, 2'd3, // lineup  0: [0, 1, 2, 3]
         2'd0, 2'd1, 2'd3, 2'd2, // lineup  1: [0, 1, 3, 2]
         2'd0, 2'd2, 2'd1, 2'd3, // lineup  2: [0, 2, 1, 3]
         2'd0, 2'd2, 2'd3, 2'd1, // lineup  3: [0, 2, 3, 1]
         2'd0, 2'd3, 2'd1, 2'd2, // lineup  4: [0, 3, 1, 2]
         2'd0, 2'd3, 2'd2, 2'd1, // lineup  5: [0, 3, 2, 1]
         2'd1, 2'd0, 2'd2, 2'd3, // lineup  6: [1, 0, 2, 3]
         2'd1, 2'd0, 2'd3, 2'd2, // lineup  7: [1, 0, 3, 2]
         2'd1, 2'd2, 2'd0, 2'd3, // lineup  8: [1, 2, 0, 3]
         2'd1, 2'd2, 2'd3, 2'd0, // lineup  9: [1, 2, 3, 0]
         2'd1, 2'd3, 2'd0, 2'd2, // lineup 10: [1, 3, 0, 2]
         2'd1, 2'd3, 2'd2, 2'd0, // lineup 11: [1, 3, 2, 0]
         2'd2, 2'd0, 2'd1, 2'd3, // lineup 12: [2, 0, 1, 3]
         2'd2, 2'd0, 2'd3, 2'd1, // lineup 13: [2, 0, 3, 1]
         2'd2, 2'd1, 2'd0, 2'd3, // lineup 14: [2, 1, 0, 3]
         2'd2, 2'd1, 2'd3, 2'd0, // lineup 15: [2, 1, 3, 0]
         2'd2, 2'd3, 2'd0, 2'd1, // lineup 16: [2, 3, 0, 1]
         2'd2, 2'd3, 2'd1, 2'd0, // lineup 17: [2, 3, 1, 0]
         2'd3, 2'd0, 2'd1, 2'd2, // lineup 18: [3, 0, 1, 2]
         2'd3, 2'd0, 2'd2, 2'd1, // lineup 19: [3, 0, 2, 1]
         2'd3, 2'd1, 2'd0, 2'd2, // lineup 20: [3, 1, 0, 2]
         2'd3, 2'd1, 2'd2, 2'd0, // lineup 21: [3, 1, 2, 0]
         2'd3, 2'd2, 2'd0, 2'd1, // lineup 22: [3, 2, 0, 1]
         2'd3, 2'd2, 2'd1, 2'd0 // lineup 23: [3, 2, 1, 0]
      };
   ...
   |batch
      @1
         ...
         *BatchNum[7:0] = $BatchNum;
         ...
         // On each round's completion edge, add 7 point(s) for every board this team won.
         ...
         $win_inc['']m5_CurTeam[7:0] = m5_repeat(m5_num_boards, ['(*BoardWon\[m5_LoopCnt\] && *BoardWinner\[m5_LoopCnt\] == m5_CurTeam ? 8'd7 : 8'd0) + ']) 8'd0;
         ...
         *failed = ! clk || 1'b0;
         ...
         \viz_js
            ...
            init() {
               ...
               let objs = {}
               objs.title = new fabric.Text("Tournament Points (7 per 4-team win)", {
