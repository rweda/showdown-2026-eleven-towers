...
\m5
   ...
   /
   / 2-TEAM TOURNAMENT RUN  (batch grid + subset seating + points scoring)
   /
   / All four enlisted teams are instantiated ONCE (the roster). Every game seats 2 of
   / them (a "2-team game"). We play every ordered seating (all 12 permutations of 2 teams
   ...
   / (g % 12, looked up in *seat_map) and a dice seed (g), so every (seating, seed) pair plays
   / once. Which 2 of the 4 teams are seated -- and in what turn order -- is chosen per game by
   // *seat_map (a runtime lookup via the per-board $lineup index), so all 4 team circuits are
   / built once yet only 2 play any given game (num_seats=2 decouples seats from the roster).
   /
   / Scoring: only the SOLE winner of a game scores, 3 point(s) each (a 2-team game is worth
   / 3). Draws (timed out) score nothing. The scoreboard under the grid shows each team's
...
\TLV
   ...
   m5_var(num_lineups, 12)        // distinct ordered seatings (permutations of 2 of the 4 teams)
   m5_var(num_seats, 2)          // players seated per game (K); < roster => subset seating
   ...
   // Seat->team map: for each of the 12 ordered seatings, the team index (0..3) seated at seats
   // 0..1, flattened as [lineup * 2 + seat]. A per-board $Lineup selects the seating at runtime.
   \SV_plus
      logic[1:0] *seat_map[0:23] = {
         2'd0, 2'd1,        // lineup  0: [0, 1]
         2'd0, 2'd2,        // lineup  1: [0, 2]
         2'd0, 2'd3,        // lineup  2: [0, 3]
         2'd1, 2'd0,        // lineup  3: [1, 0]
         2'd1, 2'd2,        // lineup  4: [1, 2]
         2'd1, 2'd3,        // lineup  5: [1, 3]
         2'd2, 2'd0,        // lineup  6: [2, 0]
         2'd2, 2'd1,        // lineup  7: [2, 1]
         2'd2, 2'd3,        // lineup  8: [2, 3]
         2'd3, 2'd0,        // lineup  9: [3, 0]
         2'd3, 2'd1,        // lineup 10: [3, 1]
         2'd3, 2'd2        // lineup 11: [3, 2]
      };
   ...
   |batch
      @1
         ...
         *BatchNum[7:0] = $BatchNum;
         ...
         // On each round's completion edge, add 3 point(s) for every board this team won.
         ...
         $win_inc['']m5_CurTeam[7:0] = m5_repeat(m5_num_boards, ['(*BoardWon\[m5_LoopCnt\] && *BoardWinner\[m5_LoopCnt\] == m5_CurTeam ? 8'd3 : 8'd0) + ']) 8'd0;
         ...
         *failed = ! clk || 1'b0;
         ...
         \viz_js
            ...
            init() {
               ...
               let objs = {}
               objs.title = new fabric.Text("Tournament Points (3 per 2-team win)", {
